...
public class ChatService(IKoalaWikiContext koala, IUserContext userContext) : FastApi
{
    public async Task CompletionsAsync(CompletionsInput input, HttpContext context)
    {
        try
        {
            ...
            Console.WriteLine($"ChatService.CompletionsAsync вызван с ChatShareMessageId: {input.ChatShareMessageId}, Question: {input.Question}");
            
            var chatShareMessage = await koala.ChatShareMessages
                .AsNoTracking()
                .FirstOrDefaultAsync(x => x.Id == input.ChatShareMessageId);
                
            Console.WriteLine($"ChatShareMessage найден: {chatShareMessage != null}");

            if (chatShareMessage == null)
            {
                throw new Exception($"Сообщение чата с ID '{input.ChatShareMessageId}' не найдено");
            }
            ...
            if (warehouse == null)
            {
                throw new Exception($"Хранилище с ID '{chatShareMessage.WarehouseId}' не найдено");
            }
            ...
            if (string.IsNullOrEmpty(document.GitPath))
            {
                throw new Exception("Путь к Git-репозиторию документа не установлен");
            }
